...
**改善点**:
- ROUGH_PLAN が `interrupt` 機構の詳細に踏み込みすぎた。IMPLEMENT.md で Router パターンへの設計変更が発生し、ROUGH_PLAN と IMPLEMENT.md の間に矛盾が生じた
- **改善策**: ROUGH_PLAN は「何をするか」（機能・ユーザー体験）に留め、「どうやるか」（実装方式）は IMPLEMENT.md に委ねる。split_plan のスキルに注意書きを追加する
...
1. **split_plan への ROUGH_PLAN 粒度ガイドの追加**: ROUGH_PLAN が実装詳細に踏み込むことを防ぐ注意書きを追加 → **本ステップで適用済み**
...
`ROUGH_PLAN.md` の粒度ガイドを追加。実装詳細ではなく機能・スコープの記述に留めるよう明示。
