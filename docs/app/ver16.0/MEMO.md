...
**検証先送り**。単体テストレベルでは `getState` をモックしているため、「ストリーム完了直後のスナップショットに最新 AIMessage が含まれるか」という LangGraph 実機挙動は検証していない。
...
**検証先送り**。`@langchain/langgraph-checkpoint-sqlite` の内部が任意の `additional_kwargs` を保持できるかは実機検証が必要。
...
**検証先送り**。実機確認でのみ違和感が判断可能。ROUGH_PLAN レビューの通り初期リリースでは見送り、必要性は実機で判断。
