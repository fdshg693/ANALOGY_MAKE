...
- `./MASTER_PLAN/PHASE7.1.md` — **一部実装済み**（ver15.0 で §1 `issue_scout` workflow を新設: `scripts/claude_loop_scout.yaml` / `.claude/skills/issue_scout/SKILL.md` / `workflow.py` の `--workflow scout` 入口を追加。§2〜§4（QUESTIONS workflow / PLAN_HANDOFF 分離 / run 単位通知）は ver15.1〜15.2 に先送り）
