...
- `./MASTER_PLAN/PHASE6.0.md` — **一部実装済み**（ver7.0 で §1 `issue_worklist.py` 新設・`claude_loop_lib/issues.py` 共通化 と §4 `/retrospective` での活用手順追記を実装。§2 `/issue_plan` 分離・§3 `--workflow auto` 導入は未着手。詳細は `MASTER_PLAN/PHASE6.0.md` 参照）
