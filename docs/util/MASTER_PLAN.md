...
- `./MASTER_PLAN/PHASE8.0.md` — **§1 実装済み**（ver16.0 で §1 `research` workflow を新設。`claude_loop_research.yaml`（8 step）・`/research_context`・`/experiment_test` SKILL・`WORKFLOW_YAML_FILES` レジストリを追加。§2 長時間コマンド委譲は ver16.1、§3 token/cost 計測は ver16.2 に持ち越し。詳細は `MASTER_PLAN/PHASE8.0.md` 参照）
