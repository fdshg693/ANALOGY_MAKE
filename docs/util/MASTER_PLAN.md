...
- `./MASTER_PLAN/PHASE4.0.md` — **部分実装**（ver4.0 でステップごとの `--model` / `--effort` 指定を実装。top-level `defaults:` と各ステップでの上書きをサポート。**未対応**: セッション継続（`continue: true` / `-r` / `--session-id` / `--output-format stream-json`）・`scripts/README.md` 新規作成 → ver4.1 以降で対応予定）
