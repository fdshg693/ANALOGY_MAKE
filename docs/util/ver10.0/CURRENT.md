# CURRENT: util ver10.0

util カテゴリのコード現況。ver9.0〜ver9.2 で `--workflow auto` 強化・`issue_worklist.py --limit` を追加した後、ver10.0 で PHASE7.0 §1「workflow YAML の step 単位 system prompt / model override」を実装した状態。`scripts/claude_loop_lib/workflow.py` に `OVERRIDE_STRING_KEYS` / `ALLOWED_STEP_KEYS` / `ALLOWED_DEFAULTS_KEYS` 定数を導入し、`commands.py` に `--system-prompt` / `--append-system-prompt` の継承ロジックを追加した。
...
### util カテゴリの ISSUES 状況（ver10.0 完了時点）
...
| `ISSUES/util/medium/test-issue-worklist-limit-omitted-returns-all.md` | medium | ready | ai |
