# CURRENT: util ver11.0

util カテゴリのコード現況。ver10.0 で PHASE7.0 §1「step 単位 system prompt / model override」を実装した後、ver11.0 で `tests/test_claude_loop.py`（1881 行・41 クラス）を `scripts/tests/` 配下 11 ファイルに分割し、Python テストをアプリ本体テスト（Vitest）から物理的に分離した状態。

- [CURRENT_scripts.md](CURRENT_scripts.md) — `scripts/` 配下の Python スクリプト・YAML ワークフロー定義・新設テストディレクトリ
- [CURRENT_skills.md](CURRENT_skills.md) — `.claude/skills/` の SKILL ファイル群・サブエージェント（ver10.0 から変更なし）
- [CURRENT_tests.md](CURRENT_tests.md) — `scripts/tests/` 配下の Python テスト構成
...
```bash
python scripts/claude_loop.py                                      # = --workflow auto（デフォルト）
...
python scripts/claude_loop.py --auto                               # 無人実行モード
...
python scripts/issue_worklist.py --format json --limit 20   # JSON 出力・20 件上限
...
### util カテゴリの ISSUES 状況（ver11.0 完了時点）

| ファイル | priority | status | assigned |
|---|---|---|---|
| `ISSUES/util/medium/issue-review-rewrite-verification.md` | medium | ready | ai |
| `ISSUES/util/medium/cli-flag-compatibility-system-prompt.md` | medium | ready | ai |
| `ISSUES/util/medium/test-issue-worklist-limit-omitted-returns-all.md` | medium | raw | ai |
| `ISSUES/util/medium/scripts構成改善.md` | medium | ready | ai |
| `ISSUES/util/low/system-prompt-replacement-behavior-risk.md` | low | ready | ai |
