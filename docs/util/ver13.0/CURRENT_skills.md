# CURRENT_skills: util ver13.0 — SKILL ファイル・サブエージェント

ver13.0 で `issue_plan` / `split_plan` / `retrospective` の 3 SKILL を更新した（`REQUESTS/AI` 参照を ISSUES 参照に置換）。その他の SKILL は ver12.0 と同一。
...
| `retrospective/SKILL.md` | 84 | ステップ 6: 振り返り。§3 冒頭で `issue_worklist.py` を呼び出し次バージョン推奨の材料を収集。ver13.0 で `REQUESTS/AI/` 整理セクションを削除 |
...
| `meta_judge/WORKFLOW.md` | 47 | 保守上の注意（3 ファイル同期義務・`--workflow auto` 実装済み）を定義 |
...
方向性確認が必要な場合の書き出し先を変更:
- 旧: `REQUESTS/AI/ に方向性確認のリクエストを書き出した上で、暫定的に既存 ISSUES 消化に倒す`
- 新: `ISSUES/{カテゴリ}/medium/ に "direction-check-ver{X.Y}.md" を作成（frontmatter: status=need_human_action, assigned=human）し、暫定的に既存 ISSUES 消化に倒す`

### `split_plan/SKILL.md`

整合性エラー時の書き出し先を変更:
- 旧: `REQUESTS/AI/ に整合性エラーとして記録し終了する`
- 新: `logs/workflow/ にエラー記録しつつ、ISSUES/{カテゴリ}/high/ に "split-plan-consistency-error-ver{X.Y}.md" を作成（frontmatter: status=need_human_action, assigned=human）して終了する`

### `retrospective/SKILL.md`

- `REQUESTS/AI/ の整理` セクション全体を**削除**（REQUESTS 廃止により不要）
