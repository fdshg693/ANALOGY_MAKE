# CURRENT: util ver14.0

util カテゴリのコード現況。ver14.0 で PHASE7.0 §6+§7+§8（retrospective からの FEEDBACK handoff・`.claude/rules/scripts.md` 新設・workflow prompt/model 評価）を一括実装した状態。

- [CURRENT_scripts.md](CURRENT_scripts.md) — `scripts/` 配下の Python スクリプト・YAML ワークフロー定義・テスト（ver14.0 では `test_claude_loop_integration.py` に `TestFeedbackInvariant` を追加）
- [CURRENT_skills.md](CURRENT_skills.md) — `.claude/skills/` の SKILL ファイル群・`.claude/rules/` の rule ファイル群（ver14.0 で `retrospective` / `issue_plan` を更新・`rules/scripts.md` 新規追加）
- [CURRENT_tests.md](CURRENT_tests.md) — `scripts/tests/` 配下の Python テスト構成（ver14.0 での追加・更新）
...
| `.claude/rules/scripts.md` | `scripts/**/*` を対象にした stable 規約（Python 前提・パス操作・CLI 引数・frontmatter/YAML 更新作法・ログ出力）。ver14.0 で新規追加 |
...
### util カテゴリの ISSUES 状況（ver14.0 完了時点）
