...
| `.claude/rules/scripts.md` | 39 | `scripts/**/*` を対象にした stable 規約（Python 前提・パス操作・CLI 引数・frontmatter/YAML 更新作法・ログ出力）。ver14.0 で新規追加。§3 に「3 ファイル間同期」と記載されているが ver15.0 で scout YAML を追加したため実態は 4 ファイル同期（`scripts/README.md` / `scripts/USAGE.md` が一次資料のため rules 本文は未更新） |
