# CURRENT_tests: util ver6.0 — テスト構成

`tests/test_claude_loop.py` は `scripts/claude_loop.py` と `scripts/claude_loop_lib/` のユニットテスト。`unittest` を使用、25 テストクラス・103 テストケース（ver6.0 では新テストの追加なし）。`pnpm test` では vitest（app カテゴリ）と unittest の合計 15 ファイル・145 テストが全パス。

`scripts/issue_status.py` のユニットテストは ver6.0 スコープ外。将来バージョンで追加予定。
...
| `TestBuildCommandWithSession` | `build_command(session_id=..., resume=...)`: `resume=True` で `-r` フラグ、`resume=False` で `--session-id` フラグ、session_id=None で両フラグなし、`--append-system-prompt` より前に配置 |
| `TestGetStepsContinue` | `get_steps()` の `continue` 受け取り: 省略→False、true/false 正規化、非 bool→`SystemExit` |
| `TestRunStepsSessionTracking` | `_run_steps()` のセッション ID 管理: `continue=false` で新規 UUID、`continue=true` で `-r` による引き継ぎ、`--start > 1` で continue 無効化・警告 |
...
- `_run_steps`（`TestRunStepsSessionTracking` 向け）
