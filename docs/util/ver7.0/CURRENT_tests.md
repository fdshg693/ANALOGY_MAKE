# CURRENT_tests: util ver7.0 — テスト構成

`tests/test_claude_loop.py` は `scripts/claude_loop.py` と `scripts/claude_loop_lib/` のユニットテスト。`unittest` を使用。ver6.1 で `TestParseFrontmatter`（5 ケース）、ver7.0 で `TestExtractStatusAssigned`（3 ケース）と `TestIssueWorklist`（8 ケース）を追加し、28 テストクラス・119 テストケース。`pnpm test` では vitest（app カテゴリ）と unittest の合計 145 テストが全パス。
...
| `TestParseFrontmatter` | **ver6.1 で追加**。`parse_frontmatter()`: 正常系/先頭`---`なし/閉じ`---`なし/YAML不正/dict以外 |
| `TestExtractStatusAssigned` | **ver7.0 で追加**。`extract_status_assigned()`: frontmatter なし→`raw/human`フォールバック、正常 frontmatter で 4-tuple 返却、無効 combo で stderr 警告 |
| `TestIssueWorklist` | **ver7.0 で追加**。`issue_worklist.py` の絞り込みロジック: ready/ai 2 件ヒット・human フィルタ除外・frontmatter なし除外・JSON 出力パース・status 単一指定・priority ミスマッチ警告・`collect()` 空 dir・text フォーマット 0 件 |
...
- `TestIssueWorklist`: `tempfile.TemporaryDirectory` で一時 `ISSUES/` ツリーを組み、`issue_worklist.REPO_ROOT` / `ISSUES_DIR` をモンキーパッチする方式。テストでは全ケースで `--category` 相当値を `collect()` に直接渡して `_default_category` のフォールバックを経由しない
