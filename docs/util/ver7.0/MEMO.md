...
- `docs/util/MASTER_PLAN/PHASE6.0.md` → §1 / §4 が本バージョンで完了した旨をチェック済みに更新する
  - ✅ **wrap_up で対応済み**: 実装進捗テーブルを冒頭に追加（§1 / §4 完了、§2 / §3 未着手、§5 部分完了を明示）
- `docs/util/ver6.0/CURRENT_scripts.md` → `issue_worklist.py` の項目追加が必要
  - ⏭️ **先送り**: `/write_current` で対応する（MEMO に記録済みのため ISSUES 起票不要）
- `docs/util/ver6.0/CURRENT_tests.md` → `TestIssueWorklist` / `TestExtractStatusAssigned` の追加反映
  - ⏭️ **先送り**: `/write_current` で対応する（同上）
