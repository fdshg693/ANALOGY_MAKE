# CURRENT_skills: util ver9.0 — SKILL ファイル・サブエージェント

`.claude/skills/` 配下の SKILL ファイルと、サブエージェントの構成。ver9.0 での SKILL 本体の変更は `.claude/skills/meta_judge/WORKFLOW.md` の保守義務追記のみ。フルワークフロー・軽量ワークフロー・ISSUE レビュー仕様は ver8.0 から変更なし。

## フルワークフロー（6 ステップ）

| ファイル | 行数 | 役割 |
|---|---|---|
| `issue_plan/SKILL.md` | 98 | ステップ 1: 前半ステップ（現状把握・ISSUE レビュー・ISSUE/MASTER_PLAN 選定・ROUGH_PLAN.md 作成・workflow 判定）。review は行わない（plan_review_agent は起動しない） |
| `split_plan/SKILL.md` | 38 | ステップ 2: 後半ステップ（REFACTOR/IMPLEMENT 作成 + plan_review_agent での review のみ）。現状把握・ISSUE 選定・ROUGH_PLAN.md 作成ロジックは `/issue_plan` に移管済み |
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `issue_plan/SKILL.md` | 98 | ステップ 1: quick でも同じ前半ステップを使用 |
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `issue_review/SKILL.md` | 99 | ISSUE レビューフェーズの**一次資料**。`/issue_plan` が参照し、スキャン → 個別レビュー → 書き換えガード → サマリ報告 の手順を定義。**直接起動しない**（`/issue_plan` からインライン展開方式） |
...
ワークフロー先頭の共通ステップ。フル・quick の両ワークフローで同一 SKILL を使用する。`--workflow auto` の phase 1 でも使用される。
...
`--workflow auto` の `_read_workflow_kind()` はこの frontmatter を読んで phase 2 の YAML を選択する。

## `/split_plan` SKILL の現在の構成（ver8.0 縮小後、変更なし）
...
| `meta_judge/WORKFLOW.md` | 48 | **ver9.0 で更新**。「保守上の注意」に 3 ファイル同期義務・`auto` 実装済み（ver9.0）の説明を追記 |

### `meta_judge/WORKFLOW.md` の保守上の注意（ver9.0 更新後）

- `claude_loop.yaml` / `claude_loop_quick.yaml` / `claude_loop_issue_plan.yaml` の `command` / `mode` / `defaults` セクションは同一内容で維持する（3 ファイル同期義務）
- `--workflow auto`（新デフォルト）は `claude_loop_issue_plan.yaml` で `/issue_plan` を先行実行し、出力された最新 `ROUGH_PLAN.md` の frontmatter `workflow:` に応じて `claude_loop.yaml` / `claude_loop_quick.yaml` の `steps[1:]` を実行する（ver9.0 実装済み）
