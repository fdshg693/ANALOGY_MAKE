...
AI応答を SSE（Server-Sent Events）でリアルタイムにストリーミング表示する状態。
...
- `sendMessage()`:
  ...
  3. `token` イベント受信時に assistant メッセージの `content` を逐次追加（同一オブジェクト参照を更新）
  4. ストリーム完了後にコンテンツが空の場合はフォールバックメッセージを設定
  5. エラー時は空の assistant メッセージを除去
- POST リクエストで SSE を受信するため `EventSource` API は使用不可 → `fetch` + `ReadableStream` + 手動 SSE パーサーで実装
- ローディング表示: `isLoading && !isStreaming` の間のみ「考え中...」を表示（ストリーミング開始後は非表示）
- 自動スクロール: `watch` で `messages.length` と最終メッセージの `content.length` を結合した文字列を監視 → `nextTick` 後にスクロール

### `app/components/ChatMessage.vue`（49行）

- Props: `role`（`'user' | 'assistant'`）, `content`（`string`）
- ロールラベル表示（"You" / "AI"）
- ユーザーメッセージ: 右寄せ・青系背景、AIメッセージ: 左寄せ・グレー背景
- `white-space: pre-wrap` で改行保持、最大幅 80%

### `app/components/ChatInput.vue`（71行）
...
### `server/utils/analogy-agent.ts`（29行）
...
- Markdown レンダリング（応答テキストのリッチ表示）
- エラー時の UI 上のフィードバック改善（エラーメッセージの表示等）
- 応答の中断・キャンセル機能
