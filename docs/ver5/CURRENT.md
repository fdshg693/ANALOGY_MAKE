...
SSE の全イベント（`token` / `done` / `error`）をクライアントで処理し、エラー発生時にチャット UI 上でフィードバックを表示する状態。
...
### `app/components/ChatMessage.vue`（55行）

- Props: `role`（`'user' | 'assistant'`）, `content`（`string`）, `isError?`（`boolean`）
- ロールラベル表示（"You" / "AI"）
- テンプレートのルート要素に `:class="[role, { error: isError }]"` で動的クラスを付与
- スタイル:
  - ユーザーメッセージ: 右寄せ・青系背景（`#dbeafe`）
