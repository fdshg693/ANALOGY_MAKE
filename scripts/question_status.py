"""QUESTION ステータス・担当の分布を表示する読み取り専用スクリプト.

Usage:
    python scripts/question_status.py              # 全カテゴリを集計
    python scripts/question_status.py <category>   # 指定カテゴリのみ集計

集計対象:
    QUESTIONS/{category}/{high,medium,low}/*.md

frontmatter 仕様は QUESTIONS/README.md を参照.
フォールバック:
...
from claude_loop_lib.questions import extract_status_assigned  # noqa: E402
...
def collect_priority(category_dir: Path, priority: str) -> Counter[tuple[str, str]]:
    ...
    for md_file in sorted(priority_dir.glob("*.md")):
        status, assigned, _fm, _body = extract_status_assigned(md_file)
        counter[(status, assigned)] += 1
    return counter


def format_priority_line(priority: str, counter: Counter[tuple[str, str]]) -> str:
    """Format a single priority row with the canonical slots always shown."""
    parts: list[str] = []
...
def main(argv: list[str]) -> int:
    if len(argv) > 1:
        category = argv[1]
        if not (QUESTIONS_DIR / category).is_dir():
            warn(f"category '{category}' not found under QUESTIONS/")
            return 0
        print_category(category)
    else:
        for category in CATEGORIES:
            if (QUESTIONS_DIR / category).is_dir():
                print_category(category)
    return 0
