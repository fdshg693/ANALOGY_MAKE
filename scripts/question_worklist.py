"""Filter QUESTIONs by assigned/status and print text or JSON.

Usage:
    python scripts/question_worklist.py
    python scripts/question_worklist.py --format json
    python scripts/question_worklist.py --category app
    python scripts/question_worklist.py --assigned human --status need_human_action

走査対象:
    QUESTIONS/{category}/{high,medium,low}/*.md
...
# QUESTION の title / summary に日本語や em-dash を含むため、Windows の cp932 既定では
...
from claude_loop_lib.questions import (  # noqa: E402
    VALID_ASSIGNED,
    VALID_STATUS,
...
def collect(category: str, assigned: str, status_list: list[str]) -> list[dict]:
    items: list[dict] = []
    category_dir = QUESTIONS_DIR / category
...
def format_text(category: str, items: list[dict],
    ...
    lines = [f"[{category}]"]
    if not items:
        lines.append("  (no matching questions)")
        return "\n".join(lines)
    for it in items:
        lines.append(
    ...
    if total is not None and limit is not None and total > limit:
        lines.append(f"(showing first {limit} of {total} questions)")
    return "\n".join(lines)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--limit", type=int, default=None,
                        help="maximum number of questions to return (default: all)")
